...
void testStergeValoriPentruCheie() {
	MD m;

	vector<TValoare> v;
	
	v = m.stergeValoriPentruCheie(3);
	assert(v.size() == 0);

	v = m.stergeValoriPentruCheie(1);
	assert(v.size() == 0);

	v = m.stergeValoriPentruCheie(6);
	assert(v.size() == 0);
